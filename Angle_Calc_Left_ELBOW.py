...
class Angle_Calc_Left_ELBOW:
    ...
    def process(self, frame, isStartSet, evaluation):
        ...
        try:
            ...
            # Get coordinates
            shoulder = [self.landmarks[mp.solutions.pose.PoseLandmark.RIGHT_SHOULDER.value].x,
                        self.landmarks[mp.solutions.pose.PoseLandmark.RIGHT_SHOULDER.value].y]
            elbow = [self.landmarks[mp.solutions.pose.PoseLandmark.RIGHT_ELBOW.value].x,
                     self.landmarks[mp.solutions.pose.PoseLandmark.RIGHT_ELBOW.value].y]
            wrist = [self.landmarks[mp.solutions.pose.PoseLandmark.RIGHT_WRIST.value].x,
                     self.landmarks[mp.solutions.pose.PoseLandmark.RIGHT_WRIST.value].y]
            ...
            cv2.putText(self.image, 'Left Elbow :', (2, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 255), 2)
            #cv2.putText(self.image, str(u'\u21'),
            ...
            # print("---****---")
            # print(angle)
            # print(self.isStartSet)
            # print(self.isevaluation)
            # print("---*---")
        ...
        mp.solutions.drawing_utils.draw_landmarks(self.image, self.results.pose_landmarks, mp.solutions.pose.POSE_CONNECTIONS,
                                  mp.solutions.drawing_utils.DrawingSpec(color=(255, 117, 66), thickness=2, circle_radius=2),
                                  mp.solutions.drawing_utils.DrawingSpec(color=(255, 66, 230), thickness=2, circle_radius=2)
                                  )
